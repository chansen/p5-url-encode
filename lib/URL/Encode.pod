...
Decodes a URL-encoded octet string.

=head2 url_decode_utf8

    $string = url_decode_utf8($octets);

Decodes a URL-encoded octet string using the UTF-8 encoding scheme.

=head2 url_encode

Encodes an octet string into a URL-encoded encoding.
...
Encodes a character string into a URL-encoded encoding using the UTF-8 encoding
scheme.
...
=head2 url_params_flat
    ...
    $set = url_params_flat($octets);
    $set = url_params_flat($octets, $utf8);

Parses a URL-encoded form data set of name/value pairs from the given octets. 
Returns an ARRAY reference containing the URL-decoded name/value pairs.

    $set = url_params_flat('foo=A&foo=B&bar=C');
    $set; # [ foo => 'A', foo => 'B', bar => 'C' ]

=head2 url_params_mixed

    $set = url_params_mixed($octets);
    $set = url_params_mixed($octets, $utf8);
...
the values for that parameter in order.
    ...
    $set = url_params_mixed('foo=A&foo=B&bar=C');
    $set; # { foo => [ 'A', 'B' ], bar => 'C' }

=head2 url_params_multi

    $set = url_params_multi($octets);
    $set = url_params_multi($octets, $utf8);

Parses a URL-encoded form data set of name/value pairs from the given octets. 
Returns a HASH reference containing the URL-decoded name/value pairs. Values 
are stored in an ARRAY reference.

    $set = url_params_multi('foo=A&foo=B&bar=C');
    $set; # { foo => [ 'A', 'B' ], bar => [ 'C' ] }
